...
# pdf 경로
pdf_path="C:\\datapare\\CL\\7283.pdf"
# excel 경로
excel_path="C:\\datapare\\CL\\7283.xlsx"
...
top=(23.58*72)/25.4
left=(67.89*72)/25.4
width=(140.15*72)/25.4
height=(318.15*72)/25.4

col1=(11.22*72)/25.4+left
col2=(16.36*72)/25.4+col1
col3=(79.68*72)/25.4+col2
col4=(10.08*72)/25.4+col3
col5=(6.85*72)/25.4+col4
col6=(12.74*72)/25.4+col5
...
# 1, 2 페이지의 column의 개수가 7개
for i in range(1, num_of_pages+1):
    ta_df = tabula.read_pdf(pdf_path, columns=p_cols, area=p_area, pages=i, **kwargs)
    df_ta=ta_df[0]
    print(len(df_ta.columns))


# 아래 네 가지 유형 중 페이지가 어떤 것에 해당하는지 파악하고, 페이지만 바꿔서 적용
# 불필요 + 필요 : 5, 10, 17, 21, 23, 27, 29, 31, 34, 37
# 필요 + 불필요 + 필요 : X
# 필요 + 불필요 : X
# 필요 : 나머지
# 불필요 : 1, 2, 3, 4, 15, 16, 20, 22, 25, 26, 28, 30, 32
# 열 삭제 되지 않은 부분이 존재
def tabu(page_p):
    temp = [5, 10, 17, 21, 23, 27, 29, 31, 34, 37]
    temp_dic_mk = [10, 21, 23, 29, 31, 34, 37]
    del_temp = [1, 2, 3, 4, 15, 16, 20, 22, 25, 26, 28, 30, 32]
    # pdf 파일 읽기
    ...
    if int(page_p) in temp:
        ...
        df_ta = ta_df[0]
        if int(page_p) in temp_dic_mk:
            del_col = len(df_ta.columns) - 1
            del_col_name = df_ta.columns[del_col]
            df_ta = df_ta.drop([del_col_name], axis=1)
