...
# pdf 경로
pdf_path="C:\\datapare\\CL\\5063.pdf"
# excel 경로
excel_path="C:\\datapare\\CL\\5063.xlsx"
...
top=(22.25*72)/25.4
left=(66.75*72)/25.4
width=(139.77*72)/25.4
height=(342.69*72)/25.4

col1=(12.55*72)/25.4+left
col2=(16.73*72)/25.4+col1
col3=(79.87*72)/25.4+col2
col4=(9.32*72)/25.4+col3
col5=(7.42*72)/25.4+col4
col6=(12.93*72)/25.4+col5
...
top=(24.34*72)/25.4
left=(68.85*72)/25.4
width=(138.25*72)/25.4
height=(342.69*72)/25.4

col1=(10.08*72)/25.4+left
col2=(16.73*72)/25.4+col1
col3=(79.87*72)/25.4+col2
col4=(11.22*72)/25.4+col3
col5=(6.85*72)/25.4+col4
col6=(13.12*72)/25.4+col5
...
d_area=(y1, x1, y2, x2)
d_cols=(col1, col2, col3, col4, col5, col6)
...
# 불필요 + 필요 : 5, 6, 8, 13, 14, 17, 21, 24, 26, 29, 31, 34, 40, 45
# 필요 + 불필요 + 필요 : X
# 필요 + 불필요 : X
# 필요 : 나머지
# 불필요 : 1, 2, 3, 4, 28, 33, 39
# 열 삭제 되지 않은 부분이 존재
def tabu(page_p):
    # 불필요 + 필요
    temp = [5, 6, 8, 13, 14, 17, 21, 24, 26, 29, 31, 34, 40, 45]
    # 불필요
    del_temp = [1, 2, 3, 4, 28, 33, 39]
    if (int(page_p) >= 1 and int(page_p) < 14):
        # pdf 파일 읽기
        ta_df = tabula.read_pdf(pdf_path, columns=p_cols, area=p_area, pages=page_p, **kwargs)
    else:
        # 14 page를 기점으로 표의 길이가 달라짐
        ta_df = tabula.read_pdf(pdf_path, columns=d_cols, area=d_area, pages=page_p, **kwargs)
