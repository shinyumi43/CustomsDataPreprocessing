...
pdf_path="C:\\datapare\\CL\\0105.pdf"
excel_path="C:\\datapare\\CL\\0105.xlsx"
...
top=(20.05*72)/25.4
left=(22.95*72)/25.4
width=(143.61*72)/25.4
height=(292.95*72)/25.4

col1=(16.45*72)/25.4+left
col2=(16.56*72)/25.4+col1
col3=(81.15*72)/25.4+col2
col4=(8.85*72)/25.4+col3
col5=(7.6*72)/25.4+col4
col6=(13*72)/25.4+col5
...
def tabu(page_p):
    ...
    # 불필요 + 필요
    if int(page_p) == 1 or int(page_p) == 25:
    ...
    # 필요 + 불필요 + 필요
    elif int(page_p) == 2 or int(page_p) == 5 or int(page_p) == 27:
        # 필요없는 부분은 삭제하고, 새로운 데이터프레임에 삽입해서 0부터 시작하도록 변경
        df_ta = ta_df[0]
        # 행의 전체 길이
        df_len = len(df_ta)
...
def remo(df_mk, df_del_len):
    ...
    for i in range(df_del_len, df_mk_len+df_del_len):
        if (df_mk[col1][i]=='Unnamed: 0' or df_mk[col1][i]=='Partida'):
            df_mk[col1][i]=''
        if (df_mk[col2][i]=='Código' or df_mk[col2][i]=='del S.A.' or df_mk[col2][i]=='Unnamed: 1'):
            df_mk[col2][i]=''
        if (df_mk[col3][i]=='Glosa' or df_mk[col3][i]=='Unnamed: 2'):
            df_mk[col3][i]=''
        if (df_mk[col4][i]=='U.A.' or df_mk[col4][i]=='Unnamed: 2' or df_mk[col4][i]=='Unnamed: 3'):
            df_mk[col4][i]=''
        if (df_mk[col5][i]=='Adv.' or df_mk[col5][i]=='Unnamed: 3' or df_mk[col5][i]=='Unnamed: 4'):
            df_mk[col5][i]=''
        if (df_mk[col6][i]=='Estad.' or df_mk[col6][i]=='Unidad' or
            df_mk[col6][i]=='Código' or df_mk[col6][i]=='Unnamed: 4' or df_mk[col6][i]=='Unnamed: 5'):
            df_mk[col6][i]=''
        df_mk=df_mk.fillna('')
